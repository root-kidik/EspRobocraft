...
std::vector<std::uint8_t> vector_from_arrays(const Arrays&... arrays)
{
    std::vector<std::uint8_t> vector;
    (vector.insert(vector.end(), arrays.begin(), arrays.end()), ...);
    return vector;
}
...
TEST(JpegBuffer, ReadOne)
{
    ...
    EXPECT_CALL(jpeg_consumer, on_jpeg(vector_from_arrays(received_buffer))).WillOnce(testing::Return());
...
TEST(JpegBuffer, ReadTwo)
{
    ...
    EXPECT_CALL(jpeg_consumer, on_jpeg(vector_from_arrays(received_buffer_begin, received_buffer_end))).WillOnce(testing::Return());

    jpeg_buffer.add(received_buffer_begin, received_buffer_begin_size);
    jpeg_buffer.add(received_buffer_begin, received_buffer_end_size);
}
